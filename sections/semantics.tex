...
message, formally defined as $\langle \textsc{send}, a_{s}, a_{r}, n \rangle$,
where $a_{s}$ and $a_{r}$ are the \emph{sender} and \emph{recipient} address
between which the value should be transfered and $n$ is the value to transfer.
To be valid, a \textsc{send} operation must be signed by the owner of $a_{s}$.
