...
\[
    \mathcal{S}_{e+1} \equiv \apply(\mathcal{S}_e, \op)
\]
where $\apply$ is \emph{apply}, the state-transition function.  We can describe
the current state of the network $\mathcal{S}_e$ as a sequence of operations
$\mathcal{L} = \op_1 \dots \op_e$ applied recursively to an initial empty state
$\varnothing$:
\[
    \mathcal{S}_e \equiv \apply(\dots \apply(\apply(\apply(\varnothing,
    \op_1), \op_2), \op_3), \dots \op_e)
\]
...
thus possible to reconstruct $\mathcal{S}$ from $\mathcal{L}$ via the $\apply$
...
\[
    f = \frac{\partial}{\partial e}\log e
\]
...
\[
    \apply(\mathcal{O}, \tx{register}{o})
    \equiv \mathcal{O} \cup \{o\}
\]
where $\apply_o$ is a sub-function of $\apply$, and defines state-transitions
in $\mathcal{O}$.
